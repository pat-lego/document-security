package io.patlego.vm.security.pdf.cert;
...
import io.patlego.vm.security.pdf.cert.exceptions.CertificateVerificationException;
import io.patlego.vm.security.pdf.cert.exceptions.RevokedCertificateException;
...
public final class CRLVerifier {
     ...
     * @param additionalCerts set of trusted root CA certificates that will be used
     *                        as "trust anchors" and intermediate CA certificates
     *                        that will be used as part of the certification chain.
     * @throws CertificateVerificationException if the certificate could not be
     *                                          verified
        ...
        try {
            ...
            for (String crlDistributionPointsURL : crlDistributionPointsURLs) {
                ...
                if (crlIssuerCert == null) {
                    throw new CertificateVerificationException("Certificate for " + crl.getIssuerX500Principal()
                            + "not found in certificate chain, so the CRL at " + crlDistributionPointsURL
            ...
            throw new CertificateVerificationException(
                    "Cannot verify CRL for certificate: " + cert.getSubjectX500Principal(), ex);
     ...
     * @throws RevokedCertificateException if the certificate was revoked at signing
     *                                     time
     */
    public static void checkRevocation(X509CRL crl, X509Certificate cert, Date signDate,
            String crlDistributionPointsURL) throws RevokedCertificateException {
        ...
        if (revokedCRLEntry != null && revokedCRLEntry.getRevocationDate().compareTo(signDate) <= 0) {
            throw new RevokedCertificateException("The certificate was revoked by CRL " + crlDistributionPointsURL
                    + " on " + revokedCRLEntry.getRevocationDate(), revokedCRLEntry.getRevocationDate());
        } else if (revokedCRLEntry != null) {
            LOG.info("The certificate was revoked after signing by CRL " + crlDistributionPointsURL + " on "
    ...
    public static List<String> getCrlDistributionPoints(X509Certificate cert) throws IOException {
        ...
        if (!(derObjCrlDP instanceof ASN1OctetString)) {
            LOG.warn("CRL distribution points for certificate subject " + cert.getSubjectX500Principal().getName()
