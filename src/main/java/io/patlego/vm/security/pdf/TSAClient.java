...
public class TSAClient {
    ...
    public TimeStampToken getTimeStampToken(InputStream content) throws IOException {
        ...
        if (timeStampToken == null) {
            ...
            throw new IOException("Response from " + url + " does not have a time stamp token, status: "
    ...
    private byte[] getTSAResponse(byte[] request) throws IOException {
        ...
        if (username != null && password != null && !username.isEmpty() && !password.isEmpty()) {
            ...
            connection.setRequestProperty("Authorization", "Basic "
                    + new String(Base64.getEncoder().encode((username + ":" + password).getBytes(contentEncoding))));
        }
