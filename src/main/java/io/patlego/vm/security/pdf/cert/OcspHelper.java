...
import io.patlego.vm.security.pdf.SigUtils;
...
public class OcspHelper {
    ...
    // SecureRandom.getInstanceStrong() would be better, but sometimes blocks on
    // Linux
     ...
     *                          used as "trust anchors" and intermediate CA
     *                          certificates that will be used as part of the
     *                          certification chain. All self-signed certificates
     *                          are considered to be trusted root CA certificates.
     *                          All the rest are considered to be intermediate CA
     *                          certificates.
     ...
     *
     * @return the OCSPResp, when the request was successful, else a corresponding
     *         exception will be thrown. Never returns null.
     *
     * @throws IOException
    ...
    /**
     * Get responder certificate. This is available after {@link #getResponseOcsp()}
     * has been called. This method should be used instead of
     * {@code basicResponse.getCerts()[0]}
     *
    ...
    /**
     * Verifies the status and the response itself (including nonce), but not the
     * signature.
     * 
    ...
    private void verifyOcspResponse(OCSPResp ocspResponse)
        ...
        if (basicResponse != null) {
            ...
            if (status instanceof RevokedStatus) {
                ...
                LOG.info("The certificate was revoked after signing by OCSP " + ocspUrl + " on "
                        + revokedStatus.getRevocationTime());
    ...
    private byte[] getKeyHashFromCertHolder(X509CertificateHolder certHolder) {
        ...
        // org.bouncycastle.cert.ocsp.CertificateID.createCertID()
    ...
    private OCSPReq generateOCSPRequest() throws OCSPException, IOException {
        ...
        try {
            certId = new CertificateID(new SHA1DigestCalculator(), new JcaX509CertificateHolder(issuerCertificate),
        ...
        Extension responseExtension = new Extension(OCSPObjectIdentifiers.id_pkix_ocsp_response, false,
